...
void initWiFi() {
  ...
  WiFi.begin(ssid,"");//앞서 지정한 SSID,PASSWORD로 연결
...
void SetCurrentTime(){//현재시간 지정
  ...
  sprintf(SC_Hour,"%d",C_Hour);//C_Hour에 지정된 시간을 Char Pointer로 바꿔서 저장.
  C_Min=timeClient.getMinutes();//C_Min에 서버에서 가져온 현재 분 지정.
  sprintf(SC_Min,"%d",C_Min);//C_Min에 지정된 시간을 String형으로 바꿔서 저장.
  //SC_Min=String(C_Min);
