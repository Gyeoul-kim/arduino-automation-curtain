...
String SC_Hour; String SC_Min;//기기 시간 String형.
...
void handleWebSocketMessage(void *arg, uint8_t *data, size_t len) {//웹소켓 메시지 해독
  ...
  if (info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT) {
    ...
    isRoutine = message.substring(message.indexOf("&")+17,message.length());//맨 마지막 숫자를 isRoutine 에 저장.
    isRoutineI=isRoutine.toInt();//if문 통한 비교를 위해 int형으로 변환.
    Serial.print("value :");
    Serial.println(isRoutine);
    if(!isRoutineI==1){//isRoutineI가 1이 아니라면.
      Speed = message.substring(0, message.indexOf("&"));//message에서 & 앞의 문자 추출
      direction = message.substring(message.indexOf("&")+1,5);//& 뒤의 문자를 추출.
      Serial.println(message);//표시
      Serial.println(Speed);
      Serial.println(direction);
      steps=2048;//한바퀴(추 후 기기설치 후 조정 예정.)
      i_speed=1000/Speed.toInt();//최대 속도는 1000. 넘길 시에는 완~전 느리게 동작할 수 있다고 공식 문서에서 밝힘.WS Packet에서 추출한 숫자로 나누어 작동. 1,2,3
      ...
      SRO_Hour = message.substring(message.indexOf("&")+5,8);//여는시각의 시를 추출함.
      SRO_Min = message.substring(message.indexOf("&")+8,11);//여는시각의 분을 추출함.
      SRC_Hour = message.substring(message.indexOf("&")+11,14);//닫는시간의 시를 추출함.
      SRC_Min = message.substring(message.indexOf("&")+14,17);//닫는시간의 분을 추출함.
...
void SetCurrentTime(){//현재시간 지정
  ...
  SC_Hour=String(C_Hour);//C_Hour에 지정된 시간을 String형으로 바꿔서 저장.
  C_Min=timeClient.getMinutes();//C_Min에 서버에서 가져온 현재 분 지정.
  SC_Min=String(C_Min);//C_Min에 지정된 시간을 String형으로 바꿔서 저장.
...
void setup() {
  ...
  server.onNotFound([](AsyncWebServerRequest *request){//잘못된 요청하면
    request->send(LittleFS, "/404.html", "text/html");//404페이즤 출력데스와~
  }
