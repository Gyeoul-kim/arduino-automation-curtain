...
int steps = 0; //스텝(회전)을 2048(한바퀴)로 설정.
...
int RO_Hour; int RO_Min; int RC_Hour; int RC_min;
*/
char *isTrigger = ""; int isTriggerI; //int inRoutine;루틴 지정 여부 체크용
char *SC_Hour = ""; char *SC_Min = ""; char *SC_Sec; char *C_Time = ""; String SC_Time = ""; //기기 시간 String형. 
int C_Hour=0; int C_Min=0; int C_Sec=0;
  ...
  if (info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT) {
    ...
    if(isTriggerI==0){
      ...
      if (strcmp(Direction, "OPN") == 0) { //입력받은 string 이 OPN이면
        ...
        stepper.move(10240);//역방향으로 한바퀴 회전.
